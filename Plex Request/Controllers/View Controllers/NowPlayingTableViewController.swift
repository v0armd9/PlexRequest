...
class NowPlayingTableViewController: UITableViewController {
    ...
    func fetchPage(_ page: Int) {
        ...
        MovieController.sharedInstance.fetchNowPlayingMovies(pageNum: page) { (results) in
                    ...
                    MovieController.fetchImageFor(movie: movie) { (image) in
                        ...
                        if imageFetchCount == totalCount {
                            DispatchQueue.main.async {
                                MovieController.sharedInstance.nowPlayingMovies = results
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "nowPlayingCell", for: indexPath) as? MovieResultTableViewCell else {return UITableViewCell()}
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toNowPlayingDetailView" {
            guard let indexPath = self.tableView.indexPathForSelectedRow else {return}
            let movie = MovieController.sharedInstance.nowPlayingMovies[indexPath.row]
            guard let url = URL(string: "https://www.fandango.com/search/"),
                var components = URLComponents(url: url, resolvingAgainstBaseURL: true)  else {return}
            let searchQueryItem = URLQueryItem(name: "q", value: movie.title)
            let modeQueryItem = URLQueryItem(name: "mode", value: "Movies")
            components.queryItems = [searchQueryItem, modeQueryItem]
            guard let finalURL = components.url else {return}
            let destinationVC = segue.destination as? MovieDetailViewController
            destinationVC?.fandangoLink = finalURL
